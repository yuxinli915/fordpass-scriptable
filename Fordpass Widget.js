...
async function createWidget() {
    ...
    //* First column
    //*****************
    let mainCol1 = await createColumn(contentStack, {
        '*setPadding': [0, 0, 0, 0],
    });

    // Vehicle Logo
    let vehicleLogoRow = await createRow(mainCol1, {
        '*centerAlignContent': null,
    });
    let vehicleLogo = await createImage(
        vehicleLogoRow,
        await getImage(runtimeData.vehicleIcon),
        { imageSize: new Size(85, 45), '*centerAlignImage': null },
    );
    ...
    //* Second column
    //************************
    let mainCol2 = await createColumn(contentStack, {
        '*setPadding': [0, 0, 0, 0],
    });
    ...
    //* Third column
    //****************
    let mainCol3 = await createColumn(contentStack, {
        '*setPadding': [0, 0, 0, 0],
    });
    ...
    let refreshTime = carData.fetchTime
        ? calculateTimeDifference(carData.fetchTime)
        : textValues.UIValues.unknown;
    let refreshLabel = await createText(infoStack, refreshTime, {
        font: Font.mediumSystemFont(sizes.detailFontSizeSmall),
    ...
    let errorLabel = await createText(infoStack, errorMsg, {
        font: Font.mediumSystemFont(sizes.detailFontSizeSmall),
        textColor: Color.red(),
    });
...
function getBgGradient() {
    ...
    grad.locations = [0, 1];
    grad.colors = [
        new Color(runtimeData.backColorGrad[0]),
        new Color(runtimeData.backColorGrad[1]),
    ];
    return grad;
}
...
async function createTitle(headerField, element, icon = undefined) {
    ...
    if (ico !== undefined) {
        ...
        let titleImg = await createImage(headerField, imgFile, {
    ...
    let txt = await createText(
        headerField,
        textValues.elemHeaders[element] + ':',
        {
            font: Font.boldSystemFont(sizes.titleFontSize),
            textColor: new Color(runtimeData.textColor1),
        },
    );
    // return headerField;
}
...
async function createProgressBar(percent) {
    ...
    const fuel = new Path();
    fuel.addRoundedRect(
        new Rect(0, 0, (sizes.barWidth * fuelLevel) / 100, sizes.barHeight),
        3,
        2,
    );
    bar.addPath(fuel);
    ...
    if (widgetConfig.useIndicators) {
        ...
        fuel25Indicator.addRoundedRect(
            new Rect(sizes.barWidth * 0.25, 1, 2, sizes.barHeight - 2),
            3,
            2,
        );
        ...
        fuel50Indicator.addRoundedRect(
            new Rect(sizes.barWidth * 0.5, 1, 2, sizes.barHeight - 2),
            3,
            2,
        );
        ...
        fuel75Indicator.addRoundedRect(
            new Rect(sizes.barWidth * 0.75, 1, 2, sizes.barHeight - 2),
            3,
            2,
        );
...
async function createFuelElement(srcField, carData) {
    ...
    let fuelHeadericon = await createImage(
        fuelHeaderRow,
        await getImage(runtimeData.fuelIcon),
        { imageSize: new Size(11, 11) },
    );
    fuelHeaderRow.addSpacer(3);
    // console.log(`fuelLevel: ${carData.fuelLevel}`);
    let lvlTxt = carData.fuelLevel
        ? carData.fuelLevel > 100
            ? 100
            : carData.fuelLevel
        : 50;
    let fuelHeadertext = await createText(
        fuelHeaderRow,
        textValues.elemHeaders['fuelTank'],
        {
            font: Font.boldSystemFont(sizes.titleFontSize),
            textColor: new Color(runtimeData.textColor1),
        },
    );
    let fuelHeadertext2 = await createText(
        fuelHeaderRow,
        ' (' + lvlTxt + '%):',
        {
            font: Font.regularSystemFont(sizes.detailFontSizeSmall),
            textColor: new Color(runtimeData.textColor1),
        },
    );
    ...
    // Fuel Level Bar
    let fuelBarCol = await createColumn(srcField, {
        '*setPadding': [0, 0, 0, 0],
        '*centerAlignContent': null,
    });
    let fuelBarRow = await createRow(fuelBarCol, {
        '*setPadding': [0, 0, 0, 0],
    });
    let fuelBarImg = await createImage(
        fuelBarRow,
        await createProgressBar(carData.fuelLevel ? carData.fuelLevel : 50),
        {
            '*centerAlignImage': null,
            imageSize: new Size(sizes.barWidth, sizes.barHeight + 3),
        },
    );

    // Fuel Distance to Empty
    let fuelBarTextRow = await createRow(fuelBarCol, {
        '*centerAlignContent': null,
        '*topAlignContent': null,
    });
    let dteInfo = carData.distanceToEmpty
        ? `    ${Math.floor(
              carData.distanceToEmpty * widgetConfig.distanceMultiplier,
          )}${widgetConfig.unitOfLength} to E`
        : textValues.errorMessages.noData;
    let fuelDteRowTxt = await createText(fuelBarTextRow, dteInfo, {
...
async function createMileageElement(srcField, carData) {
    ...
    elem.addSpacer(2);
    let value = carData.odometer
        ? `${Math.floor(carData.odometer * widgetConfig.distanceMultiplier)}${
              widgetConfig.unitOfLength
          }`
        : textValues.errorMessages.noData;
    // console.log(`odometer: ${value}`);
    let txt = await createText(elem, value, {
        font: Font.regularSystemFont(sizes.detailFontSizeSmall),
        textColor: new Color(runtimeData.textColor2),
    });
    srcField.addSpacer(3);
...
async function createDoorElement(srcField, carData, countOnly = false) {
    const styles = {
        normTxt: {
            font: Font.mediumSystemFont(sizes.detailFontSizeMedium),
            textColor: new Color(runtimeData.textColor2),
        },
        statOpen: {
            font: Font.heavySystemFont(sizes.detailFontSizeMedium),
            textColor: new Color('FF5733'),
        },
        statClosed: {
            font: Font.heavySystemFont(sizes.detailFontSizeMedium),
    ...
    if (countOnly) {
        ...
        if (carData.statusDoors) {
            countOpenDoors = Object.values(carData.statusDoors).filter(
                (door) => door === true,
            ).length;
            value =
                countOpenDoors == 0
                    ? textValues.UIValues.closed
                    : `${countOpenDoors} ${textValues.UIValues.open}`;
        }
        ...
        let row1LfStatTxt = await createText(
            dataRow1Fld,
            carData.statusDoors['leftFront'] ? openSymbol : closedSymbol,
            carData.statusDoors['leftFront']
                ? styles.statOpen
                : styles.statClosed,
        );
        let row1LfTxt2 = await createText(
            dataRow1Fld,
            ')' + ' | ',
            styles.normTxt,
        );
        let row1RfTxt1 = await createText(dataRow1Fld, 'RF (', styles.normTxt);
        let row1RfStatTxt = await createText(
            dataRow1Fld,
            carData.statusDoors['rightFront'] ? openSymbol : closedSymbol,
            carData.statusDoors['rightFront']
                ? styles.statOpen
                : styles.statClosed,
        );
        ...
        let row2RfStatTxt = await createText(
            dataRow2Fld,
            carData.statusDoors['leftRear'] ? openSymbol : closedSymbol,
            carData.statusDoors['leftRear']
                ? styles.statOpen
                : styles.statClosed,
        );
        let row2RfTxt2 = await createText(
            dataRow2Fld,
            ')' + ' | ',
            styles.normTxt,
        );
        let row2RrTxt1 = await createText(dataRow2Fld, 'RR (', styles.normTxt);
        let row2RrStatTxt = await createText(
            dataRow2Fld,
            carData.statusDoors['rightRear'] ? openSymbol : closedSymbol,
            carData.statusDoors['rightRear']
                ? styles.statOpen
                : styles.statClosed,
        );
        ...
        if (carData.statusDoors['tailgate'] !== undefined) {
            ...
            let row3TgTxt1 = await createText(
                dataRow3Fld,
                '       TG (',
                styles.normTxt,
            );
            let row3TgStatTxt = await createText(
                dataRow3Fld,
                carData.statusDoors['tailgate'] ? openSymbol : closedSymbol,
                carData.statusDoors['tailgate']
                    ? styles.statOpen
                    : styles.statClosed,
            );
...
async function createWindowElement(srcField, carData, countOnly = false) {
    ...
    if (countOnly) {
        ...
        if (carData.statusWindows) {
            countOpenWindows = Object.values(carData.statusWindows).filter(
                (window) => window === true,
            ).length;
            value =
                countOpenWindows == 0
                    ? textValues.UIValues.closed
                    : `${countOpenWindows} ${textValues.UIValues.open}`;
        }
        let cntOpenTxt = await createText(dataRow1Fld, value, styles.normTxt);
    } else {
        let row1LfTxt1 = await createText(dataRow1Fld, 'LF (', styles.normTxt);
        let row1LfStatTxt = await createText(
            dataRow1Fld,
            carData.statusWindows['leftFront'] ? openSymbol : closedSymbol,
            carData.statusWindows['leftFront']
                ? styles.statOpen
                : styles.statClosed,
        );
        let row1LfTxt2 = await createText(
        ...
        let row1RfStatTxt = await createText(
            dataRow1Fld,
            carData.statusWindows['rightFront'] ? openSymbol : closedSymbol,
            carData.statusWindows['rightFront']
                ? styles.statOpen
                : styles.statClosed,
        );
        let row1RfTxt2 = await createText(dataRow1Fld, ')', styles.normTxt);
        ...
        let row2RfStatTxt = await createText(
            dataRow2Fld,
            carData.statusWindows['leftRear'] ? openSymbol : closedSymbol,
            carData.statusWindows['leftRear']
                ? styles.statOpen
                : styles.statClosed,
        );
        let row2RfTxt2 = await createText(
        ...
        let row2RrStatTxt = await createText(
            dataRow2Fld,
            carData.statusWindows['rightRear'] ? openSymbol : closedSymbol,
            carData.statusWindows['rightRear']
                ? styles.statOpen
                : styles.statClosed,
        );
        let row2RrTxt2 = await createText(dataRow2Fld, ')', styles.normTxt);
...
async function createPositionElement(srcField, carData) {
    ...
    let value = carData.position
        ? `${carData.position}`
        : textValues.errorMessages.noData;
    let text = await createText(dataFld, value, {
        url: url,
        font: Font.mediumSystemFont(sizes.detailFontSizeMedium),
        textColor: new Color(runtimeData.textColor2),
        lineLimit: 2,
...
async function createLockStatusElement(srcField, carData) {
    ...
    let dataFld = await createRow(srcField);
    let value = carData.lockStatus
        ? carData.lockStatus
        : textValues.errorMessages.noData;
    let text = await createText(
        dataFld,
        value,
        carData.lockStatus !== undefined && carData.lockStatus === 'LOCKED'
            ? styles.statClosed
            : styles.statOpen,
    );
    srcField.addSpacer(offset);
...
async function fetchToken() {
    ...
    let req = new Request(
        'https://fcis.ice.ibmcloud.com/v1.0/endpoint/default/token',
    );
    req.headers = {
...
async function fetchRawData() {
    ...
    let req = new Request(
        `https://usapi.cv.ford.com/api/vehicles/v4/${vin}/status`,
    );
    req.headers = {
    ...
    try {
        ...
        if (data == 'Access Denied') {
            console.log(
                'fetchRawData: Auth Token Expired. Fetching new token and fetch raw data again',
            );
            let result = await fetchToken();
            if (result && result == textValues.errorMessages.invalidGrant) {
                return result;
...
async function sendVehicleCmd(cmd_type = '') {
    ...
    for (const cmd in cmds) {
        let isLastCmd =
            !multiCmds || (multiCmds && cmds.length == parseInt(cmd) + 1);
        ...
        try {
            ...
            if (data == 'Access Denied') {
                console.log(
                    'sendVehicleCmd: Auth Token Expired. Fetching new token and fetch raw data again',
                );
                let result = await fetchToken();
                if (result && result == textValues.errorMessages.invalidGrant) {
                    console.log(`sendVehicleCmd(${cmd_type}): ${result}`);
            ...
            if (data.status) {
                console.log(
                    `sendVehicleCmd(${cmd_type}) Status Code (${data.status})`,
                );
                if (data.status != 200) {
                    wasError = true;
                    if (widgetConfig.debugMode) {
                        console.log(
                            'Debug: Error while receiving vehicle data',
                        );
                        console.log(data);
                    }
                    if (data.status == 590) {
                        ...
                        outMsg = {
                            title: `${cmd_type.toUpperCase()} Command`,
                            message: `There was an error sending the command to the vehicle!\n\Error: ${data.status}`,
                        };
                    }
                } else {
                    outMsg = {
                        title: `${cmd_type.toUpperCase()} Command`,
                        message: `Vehicle Received Command Successfully`,
                    };
                }
            }
...
function checkUserData(cred) {
    if (widgetConfig.storeCredentialsInKeychain) {
        ...
        !widgetConfig.storeCredentialsInKeychain &&
        userData[cred] != ''
    ) {
...
async function fetchCarData() {
    ...
    if (
        rawData == textValues.errorMessages.invalidGrant ||
        rawData == textValues.errorMessages.connectionErrorOrVin ||
        rawData == textValues.errorMessages.unknownError ||
        rawData == textValues.errorMessages.noVin ||
        rawData == textValues.errorMessages.noCredentials
    ) {
    ...
    //ignition status
    carData.ignitionStatus = vehicleStatus.ignitionStatus
    ...
    carData.alarmStatus = vehicleStatus.alarm
        ? vehicleStatus.alarm.value === 'SET'
            ? 'On'
            : 'Off'
        : 'Off';

    //Battery info
    carData.batteryStatus =
        vehicleStatus.battery && vehicleStatus.battery.batteryHealth
            ? vehicleStatus.battery.batteryHealth.value
            : textValues.UIValues.unknown;
    carData.batteryLevel =
        vehicleStatus.battery && vehicleStatus.battery.batteryStatusActual
    ...
    carData.deepSleepMode = vehicleStatus.deepSleepInProgess
        ? vehicleStatus.deepSleepInProgess.value === 'true'
        : undefined;

    // Whether Vehicle is currently installing and OTA update (OTA) | Supported Vehicles Only
    carData.firmwareUpdating = vehicleStatus.firmwareUpgInProgress
        ? vehicleStatus.firmwareUpgInProgress.value === 'true'
        : undefined;
    ...
    carData.statusWindows = {
        ...
        ].includes(windows.driverWindowPosition.value),
        rightFront: ![
            'Fully_Closed',
            'Fully closed position',
            'Undefined window position',
        ].includes(windows.passWindowPosition.value),
        leftRear: ![
            'Fully_Closed',
            'Fully closed position',
            'Undefined window position',
        ].includes(windows.rearDriverWindowPos.value),
        rightRear: ![
            'Fully_Closed',
            'Fully closed position',
            'Undefined window position',
        ].includes(windows.rearPassWindowPos.value),
    };
...
function isLocalDataFreshEnough() {
    ...
    if (
        localData &&
        Date.now() - localData.fetchTime < 60000 * widgetConfig.refreshInterval
    ) {
        return true;
    } else {
        return false;
...
async function getPosition(data) {
    let loc = await Location.reverseGeocode(
        parseFloat(data.gps.latitude),
        parseFloat(data.gps.longitude),
    );
...
function calculateTimeDifference(oldTime) {
    ...
    if (Math.floor((diffMs % 86400000) / 3600000) >= 1) {
        ...
        return `${textValues.UIValues.precedingAdverb} ${diff} ${
            textValues.UIValues.hour
        }${diff == 1 ? '' : textValues.UIValues.plural} ${
            textValues.UIValues.subsequentAdverb
        }`;
    }
    if (Math.round(((diffMs % 86400000) % 3600000) / 60000) >= 1) {
        let diff = Math.round(((diffMs % 86400000) % 3600000) / 60000);
        return `${textValues.UIValues.precedingAdverb} ${diff} ${
            textValues.UIValues.minute
        }${diff == 1 ? '' : textValues.UIValues.plural} ${
            textValues.UIValues.subsequentAdverb
        }`;
    }
...
function calculateDurationLeasing() {
    ...
    let duration = lsb
        ? (Date.now() - dls.getTime()) / (1000 * 60 * 60 * 24)
        : null;
...
function _mapMethodsAndCall(inst, options) {
    Object.keys(options).forEach((key) => {
        if (key.indexOf('*') !== -1) {
            ...
            if (!(key in inst)) {
                throw new Error(
                    `Method "${key}()" is not applicable to instance of ${_getObjectClass(
                        inst,
                    )}`,
                );
            }
            ...
            if (!(key in inst)) {
                throw new Error(
                    `Property "${key}" is not applicable to instance of ${_getObjectClass(
                        inst,
                    )}`,
                );
            }
